...
class FileUploadHandler {
    ...
    formatDate(dateString) {
        ...
        // 處理 6 位數格式 YYMMDD (例如 261225 -> 2026-12-25)
        if (/^\d{6}$/.test(str)) {
            ...
            if (mNum >= 1 && mNum <= 12 && dNum >= 1 && dNum <= 31) {
                const date = new Date(parseInt(year, 10), mNum - 1, dNum);
                if (!isNaN(date)) return date.toISOString().split('T')[0];
            }
        }

        // 處理 5 位數格式 YYDDD (例如 20725 -> 2025 年第 207 天的可能解讀)
        if (/^\d{5}$/.test(str)) {
            const year = '20' + str.substring(0, 2);
            const dayOfYear = parseInt(str.substring(2), 10);
            if (dayOfYear >= 1 && dayOfYear <= 366) {
                const date = new Date(parseInt(year, 10), 0, dayOfYear);
                return date.toISOString().split('T')[0]; // 返回 YYYY-MM-DD
            }

            // 若無法視為 day-of-year，嘗試另一種簡寫解釋 (例如 20725 -> 2025-07-25)
            if (str.startsWith('207')) {
                const month = str.substring(2, 3); // e.g. '7'
                const day = str.substring(3);
                const candidate = `20${str.substring(0,2)}-${month.padStart(2,'0')}-${day.padStart(2,'0')}`;
                if (this.isValidDate(candidate)) return candidate;
            }
        }
...
123,271225,1,3.8,0.285855,0.028433
123,271225,2,3.8,0.293352,0.471766
123,271225,3,9.55,0.220223,0.062638
123,271225,4,4.15,0.284407,0.561968`;
